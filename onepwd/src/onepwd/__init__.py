...
class OnePwd(object):
  ...
  def update_s3_values_of_standard_s3_item(self, title, vault=None, ACCESS_KEY=None, ACCESS_SECRET=None,
                                           BUCKET_NAME=None, ENDPOINT_URL=None):
    fields_to_change = ""
    if ACCESS_KEY is not None:
  ...
  def signin(self, secret, shorthand):
    ...
    child = pexpect.spawn(
      f"{self.op} account add --signin --address {secret['signin_address']} --email {secret['email']} --secret-key {secret['secret_key']} --raw --shorthand={shorthand} {session_flag}",
      env=os.environ)
    ...
    if token.startswith('[ERROR]'):
      raise SigninError(
        f'"{token}" - Please check email, password, subdomain, secret key, 2FA token, and your system time.')
    return token
